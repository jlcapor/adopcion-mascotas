			...
			>
				<div className="w-full px-7">
					<Link href="/" className="flex items-center" onClick={() => setOpen(false)}>
						<Icons.pet className="mr-2 size-7" aria-hidden="true" />
						<span className="font-bold">PetFriendly</span>
						<span className="sr-only">Home</span>
					</Link>
					<div className="flex items-start justify-center gap-2 flex-col mt-10">
						{items.map((item, index) => {
							...
							return item.href ? (
								...
								>
									<span
										className={cn(
											'group flex w-60 items-center rounded-md border border-transparent px-2 py-1 mr-6 hover:bg-muted hover:text-foreground text-sm font-semibold',
